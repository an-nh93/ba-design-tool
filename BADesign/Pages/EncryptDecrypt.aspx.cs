...
WHERE T.IsActiveTransaction = 1";
        ...
        private static string BuildDemoResetSql(List<EmployeeForScript> employees, DemoResetConfig cfg)
        {
            ...
            sb.AppendLine("        UPDATE T SET T.BasicSalary=B.BasicSalary FROM #B B JOIN dbo.Staffing_Transactions T WITH (ROWLOCK, UPDLOCK) ON T.EmployeeID=B.EmployeeID WHERE B.BasicSalary IS NOT NULL;");
            sb.AppendLine("    END");
